...
def submit_query(queries):
    ...
    pevents = pevents.groupby(pd.Grouper(key='event_time', freq='60s')).agg('count')
    pevents.columns = ['count']
    pevents = pevents.reset_index()
    
    gevents = gevents.groupby(pd.Grouper(key='event_time', freq='60s')).agg('count')
    gevents.columns = ['count']
    gevents = gevents.reset_index()
